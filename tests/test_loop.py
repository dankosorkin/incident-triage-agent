...
def test_run_agent_dispatches_sql_tool_and_returns_final_answer(fake_provider, monkeypatch):
    monkeypatch.setitem(loop.TOOL_DISPATCH, "sql_tool", lambda args: [{"count": 10}])
...
def test_run_agent_handles_both_tools_in_one_turn(fake_provider, monkeypatch):
    monkeypatch.setitem(loop.TOOL_DISPATCH, "sql_tool", lambda args: "sql result")
    monkeypatch.setitem(loop.TOOL_DISPATCH, "rag_tool", lambda args: "rag result")
...
def test_run_agent_stops_at_max_iterations(fake_provider, monkeypatch):
    monkeypatch.setitem(loop.TOOL_DISPATCH, "sql_tool", lambda args: "result")
